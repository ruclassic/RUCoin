...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #RUCoinTEST2\r");
            Send(hSocket, "WHO #RUCoinTEST2\r");
        } else {
            // randomly join #RUCoin00-#RUCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #RUCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #RUCoin%02d\r", channel_number).c_str());
        }
