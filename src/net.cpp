...
static const char *strDNSSeed[][2] = {
    {"104.131.160.190", "104.131.160.190"},
	{"138.197.136.213", "138.197.136.213"},
	{"139.59.122.234", "139.59.122.234"},
	{"198.38.93.100", "198.38.93.100"}
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("RUCoin-dnsseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. RUCoin is probably already running."), addrBind.ToString().c_str());
        else
